...
const LazyVehiclePhoto = ({ vehicleId, model, style }: { vehicleId: string; model: string; style?: any }) => {
  ...
  if (loading) {
    return (
      <View style={[styles.photoFallback, style, { justifyContent: 'center' }]}>
  ...
  if (hasError || !base64Photo) {
    return (
      <View style={[styles.photoFallback, style]}>
        <Car color="#3b82f6" size={28} opacity={0.6} />
      </View>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ backgroundColor: '#FFF', paddingBottom: 10 }}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.metricsContainer} contentContainerStyle={styles.metricsContent}>
          ...
          <View style={[styles.metricCard, { borderColor: '#10b981' }]}>
            <Text style={[styles.metricValue, { color: '#10b981' }]}>{stats.active}</Text>
            <Text style={styles.metricLabel}>Active</Text>
          </View>
          <View style={[styles.metricCard, { borderColor: '#f59e0b' }]}>
            <Text style={[styles.metricValue, { color: '#f59e0b' }]}>{stats.maintenance}</Text>
            <Text style={styles.metricLabel}>Maintenance</Text>
          </View>
          <View style={[styles.metricCard, { borderColor: '#ef4444' }]}>
            <Text style={[styles.metricValue, { color: '#ef4444' }]}>{stats.inspectionsDue}</Text>
            ...
            return (
              <View style={styles.card}>
                <View style={styles.cardBodyRow}>
                  <LazyVehiclePhoto vehicleId={item.id} model={item.model} style={styles.thumbnail} />
      ...
      <View style={styles.paginationRow}>
        ...
        >
          <ChevronLeft color={currentPage === 1 ? "#cbd5e1" : "#1e293b"} size={20} />
        </TouchableOpacity>
        ...
        >
          <ChevronRight color={currentPage === totalPages ? "#cbd5e1" : "#1e293b"} size={20} />
        </TouchableOpacity>
      </View>
          ...
          {selectedVehicle && (
            <ScrollView contentContainerStyle={styles.modalScroll}>
              <View style={{ alignItems: 'center', marginVertical: 20 }}>
                <LazyVehiclePhoto vehicleId={selectedVehicle.id} model={selectedVehicle.model} style={{ width: 130, height: 130, borderRadius: 12 }} />
          ...
          <ScrollView contentContainerStyle={styles.modalScroll}>
            ...
            {editVehicleOpen && selectedVehicle && (
              <View style={styles.imagePreviewWrapper}>
                ...
                <LazyVehiclePhoto vehicleId={selectedVehicle.id} model={selectedVehicle.model} style={styles.modalLivePreview} />
              </View>
            )}
          ...
          <View style={styles.pickerMenu}>
            {['active', 'maintenance', 'inactive'].map((s) => (
              <TouchableOpacity key={s} style={styles.pickerItem} onPress={() => { 
                ...
                <Text style={{ textTransform: 'capitalize', fontSize: 16, color: '#1e293b' }}>{s}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
          ...
          <View style={styles.pickerMenu}>
            <ScrollView style={{ maxHeight: 300 }}>
              ...
              <TouchableOpacity style={styles.pickerItem} onPress={() => { 
                ...
                <Text style={{ fontSize: 16, color: '#64748b', fontWeight: '500' }}>Unassigned (-)</Text>
              </TouchableOpacity>
              ...
              {employees.map((emp) => (
                <TouchableOpacity key={emp.id} style={styles.pickerItem} onPress={() => { 
                  ...
                  <Text style={{ fontSize: 16, color: '#1e293b' }}>{emp.name}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 16, backgroundColor: Colors.background, borderBottomWidth: 1, borderBottomColor: "#e2e8f0" },
  ...
  addButtonText: { color: "#FFF", fontWeight: "600", fontSize: 13, marginLeft: 4 },
  metricsContainer: { paddingLeft: 16, paddingVertical: 10, backgroundColor: Colors.background },
  metricsContent: { paddingRight: 32 },
  metricCard: { minWidth: 100, padding: 12, marginRight: 12, borderRadius: 10, borderWidth: 1, backgroundColor: Colors.background },
  metricValue: { fontSize: 18, fontWeight: "700", color: Colors.surface },
  metricLabel: { fontSize: 11, color: "#64748b", marginTop: 2 },
  searchContainer: { flexDirection: "row", alignItems: "center", backgroundColor: "#FFF", margin: 16, paddingHorizontal: 12, borderRadius: 8, borderWidth: 1, borderColor: "#e2e8f0", height: 44 },
  searchIcon: { marginRight: 8 },
  searchInput: { flex: 1, fontSize: 14, color: "#0f172a" },
  errorBanner: { flexDirection: "row", alignItems: "center", backgroundColor: "#fee2e2", marginHorizontal: 16, padding: 10, borderRadius: 6 },
  errorText: { color: "#b91c1c", marginLeft: 8, fontSize: 13, flex: 1 },
  listContent: { paddingHorizontal: 16, paddingBottom: 32 },
  card: { backgroundColor:Colors.background, borderRadius: 12, borderWidth: 1, borderColor: "#e2e8f0", padding: 14, marginBottom: 12 },
  cardBodyRow: { flexDirection: "row" },
  thumbnail: { width: 64, height: 64, borderRadius: 8 },
  photoFallback: { width: 64, height: 64, borderRadius: 8, backgroundColor: "#eff6ff", alignItems: "center", justifyContent: "center" },
  ...
  cardTitle: { fontSize: 15, fontWeight: "600", color: Colors.surface, flex: 1, marginRight: 8 },
  statusBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 },
  statusBadgeText: { fontSize: 10, fontWeight: "700" },
  statusActive: { backgroundColor: "#dcfce7" },
  statusActiveText: { color: "#15803d" },
  statusMaint: { backgroundColor: "#fef3c7" },
  statusMaintText: { color: "#b45309" },
  statusInactive: { backgroundColor: "#f1f5f9" },
  statusInactiveText: { color: "#475569" },
  cardSubtitle: { fontSize: 13, color: "#64748b" },
  assignedText: { fontSize: 12, color: "#475569", fontWeight: "500", marginTop: 2 },
  cardFooter: { flexDirection: "row", alignItems: "center", marginTop: 12, paddingTop: 10, borderTopWidth: 1, borderTopColor: "#f1f5f9" },
  mileageText: { fontSize: 12, fontWeight: "600", color: "#64748b" },
  actionRow: { flexDirection: "row" },
  actionIcon: { paddingHorizontal: 8, paddingVertical: 4, marginLeft: 8 },
  paginationRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 16, borderTopWidth: 1, borderTopColor: "#e2e8f0", backgroundColor: "#FFF" },
  pageButton: { padding: 6, borderRadius: 6, borderWidth: 1, borderColor: "#cbd5e1" },
  disabledButton: { opacity: 0.4 },
  pageText: { fontSize: 13, color: "#334155", fontWeight: "500" },
  ...
  fieldLabel: { fontSize: 13, fontWeight: "600", color: "#334155", marginTop: 14, marginBottom: 6 },
  input: { borderWidth: 1, borderColor: "#cbd5e1", borderRadius: 8, padding: 10, fontSize: 14, color: "#0f172a", backgroundColor: "#f8fafc", minHeight: 44 },
  ...
  imagePreviewWrapper: { marginTop: 16, padding: 10, backgroundColor: "#f8fafc", borderRadius: 8, borderWidth: 1, borderColor: "#e2e8f0" },
  ...
  formErrorText: { color: "#ef4444", fontWeight: "600", marginBottom: 12, fontSize: 13 },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", alignItems: "center" },
  pickerMenu: { backgroundColor: "#FFF", width: width * 0.8, borderRadius: 12, paddingVertical: 8, elevation: 5, shadowColor: "#000", shadowOpacity: 0.15, shadowRadius: 10 },
  pickerItem: { paddingVertical: 14, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
});