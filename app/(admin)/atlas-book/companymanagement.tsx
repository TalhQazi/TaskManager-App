...
function createStyles(colors: ReturnType<typeof buildColors>) {
  return StyleSheet.create({
    ...
    kpiCard: { backgroundColor: colors.cardBg, width: (width - 44) / 2, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14, marginBottom: 12 },
    kpiHeaderRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 6 },
    kpiTitle: { fontSize: 11, fontWeight: "700", color: colors.textSecondary },
    kpiValue: { fontSize: 18, fontWeight: "800", color: colors.text, marginVertical: 4 },
    kpiSubtitle: { fontSize: 10, color: colors.textMuted },
    trendBadge: { paddingHorizontal: 5, paddingVertical: 1, borderRadius: 4 },
    trendPos: { backgroundColor: colors.successBg },
    trendNeg: { backgroundColor: colors.dangerBg },
    trendText: { fontSize: 9, fontWeight: "700" },
    trendTextPos: { color: colors.successText },
    trendTextNeg: { color: colors.dangerText },
    boxCard: { backgroundColor: colors.cardBg, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 16, marginBottom: 16 },
    boxHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", borderBottomWidth: 1, borderBottomColor: colors.borderLight, paddingBottom: 10, marginBottom: 12 },
    boxHeaderTitle: { fontSize: 13, fontWeight: "800", color: colors.text, letterSpacing: 0.5 },
    listHeaderTitle: { fontSize: 11, fontWeight: "900", color: colors.text, textTransform: "uppercase", letterSpacing: 0.5 },
    timeframeBadge: { backgroundColor: colors.borderLight, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 },
    timeframeText: { fontSize: 9, color: colors.textSecondary, textTransform: "uppercase" },
    chartContainer: { height: 160, flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end", paddingTop: 10, paddingHorizontal: 4 },
    chartColumn: { alignItems: "center", flex: 1 },
    barTrack: { height: "100%", width: "100%", flexDirection: "row", justifyContent: "center", alignItems: "flex-end", position: "relative" },
    chartBar: { width: 6, marginHorizontal: 1, borderTopLeftRadius: 2, borderTopRightRadius: 2 },
    lineDot: { position: "absolute", width: 6, height: 6, borderRadius: 3, backgroundColor: colors.primary, zIndex: 2 },
    chartXLabel: { fontSize: 9, color: colors.textMuted, marginTop: 6 },
    chartLegend: { flexDirection: "row", justifyContent: "center", marginTop: 14, paddingTop: 10, borderTopWidth: 1, borderTopColor: colors.borderLight },
    legendItem: { flexDirection: "row", alignItems: "center", marginHorizontal: 10 },
    legendIndicator: { width: 10, height: 10, borderRadius: 2, marginRight: 4 },
    legendText: { fontSize: 10, color: colors.textSecondary },
    leasedBadge: { backgroundColor: colors.successBg, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 },
    leasedBadgeText: { fontSize: 10, color: colors.successText, fontWeight: "700" },
    heatmapContainer: { marginVertical: 4 },
    heatmapRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
    heatmapRowName: { fontSize: 12, fontWeight: "700", color: colors.textSecondary, width: 90 },
    heatmapCellsGrid: { flex: 1, flexDirection: "row", justifyContent: "space-between" },
    heatmapCell: { height: 28, flex: 1, marginHorizontal: 3, borderRadius: 6 },
    heatmapLegend: { flexDirection: "row", justifyContent: "flex-end", marginTop: 8, paddingTop: 10, borderTopWidth: 1, borderTopColor: colors.borderLight },
    listContainer: { marginTop: 2 },
    listItem: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: colors.borderLight },
    lastItem: { borderBottomWidth: 0, paddingBottom: 0 },
    listLabel: { fontSize: 12, color: colors.textSecondary, flex: 1, paddingRight: 8 },
    listValue: { fontSize: 12, fontWeight: "700", color: colors.text },
    alertStack: { gap: 8 },
    alertItem: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 12, backgroundColor: colors.dangerBg, borderWidth: 1, borderColor: colors.border, borderRadius: 8 },
    alertText: { fontSize: 12, color: colors.dangerText, fontWeight: "500" },
    taskItem: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: colors.borderLight },
    taskLabel: { fontSize: 12, color: colors.textSecondary, flex: 1, paddingRight: 8 },
  });
}
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.kpiGrid}>
          <View style={styles.kpiCard}>
            <View style={styles.kpiHeaderRow}>
              <Text style={styles.kpiTitle}>Company Balance</Text>
            </View>
            <Text style={styles.kpiValue} numberOfLines={1} adjustsFontSizeToFit>${kpiData.balance.toLocaleString()}</Text>
            <Text style={styles.kpiSubtitle} numberOfLines={1}>Scope: {activeEntity.name}</Text>
          </View>

          <View style={styles.kpiCard}>
            <View style={styles.kpiHeaderRow}>
              <Text style={styles.kpiTitle}>Monthly Income</Text>
            </View>
            <Text style={styles.kpiValue} numberOfLines={1} adjustsFontSizeToFit>${kpiData.income.toLocaleString()}</Text>
            <Text style={styles.kpiSubtitle} numberOfLines={1}>Cleared revenue deposits</Text>
          </View>

          <View style={styles.kpiCard}>
            <View style={styles.kpiHeaderRow}>
              <Text style={styles.kpiTitle}>Monthly Expenses</Text>
            </View>
            <Text style={styles.kpiValue} numberOfLines={1} adjustsFontSizeToFit>${kpiData.expenses.toLocaleString()}</Text>
            <Text style={styles.kpiSubtitle} numberOfLines={1}>Operating overhead burns</Text>
          </View>

          <View style={styles.kpiCard}>
            <View style={styles.kpiHeaderRow}>
              <Text style={styles.kpiTitle}>Occupancy Rate</Text>
              <View style={[styles.trendBadge, styles.trendPos]}>
                <Text style={[styles.trendText, styles.trendTextPos]}>+2.4%</Text>
              </View>
            </View>
            <Text style={styles.kpiValue} numberOfLines={1} adjustsFontSizeToFit>{kpiData.occupancy}%</Text>
            <Text style={styles.kpiSubtitle} numberOfLines={1}>Lease rollover check</Text>
          </View>
        </View>
        ...
        <View style={styles.boxCard}>
          ...
          <View style={styles.chartLegend}>
            ...
            <View style={styles.legendItem}><View style={[styles.legendIndicator, { backgroundColor: colors.primary }]} /><Text style={styles.legendText}>Net Cash</Text></View>
          </View>
        </View>
