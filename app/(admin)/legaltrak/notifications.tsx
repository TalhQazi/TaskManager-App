...
interface LegalNotification {
  ...
  title: string;
  message?: string;
  type?: "Alert" | "Reminder" | "System" | string;
  isRead?: "Yes" | "No" | string;
  createdAt?: string;
  updatedAt?: string;
}

interface ApiResponse {
  items?: LegalNotification[];
}
...
export default function NotificationsScreen() {
  ...
  const [activePicker, setActivePicker] = useState<{ type: "type" | "isRead"; options: { label: string; value: string }[] } | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      const res = await listResource<ApiResponse | LegalNotification[]>("legal/notifications");
      ...
      Alert.alert("Error", "Failed to load notifications from backend services.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const payload: any = { ...formData };
      Object.keys(payload).forEach((k) => { if (payload[k] === "") delete payload[k]; });
      await updateResource<LegalNotification>("legal/notifications", targetId, payload);
      setEditOpen(false);
      setSelectedItem(null);
      resetForm();
      loadData();
    } catch {
      Alert.alert("Error", "Failed to update target notification record metrics safely.");
    } finally {
      setIsSubmitting(false);
    }
  };

  const openDeleteConfirmation = (item: LegalNotification) => {
    const targetId = item.id || item._id || "";
    Alert.alert("Delete Notification", `Are you sure you want to delete ${item.notificationNumber}? This cannot be undone.`, [
      { text: "Cancel", style: "cancel" },
      { text: "Delete Permanently", style: "destructive", onPress: async () => {
          try {
            await deleteResource("legal/notifications", targetId);
            loadData();
          } catch {
  ...
  const openPickerModal = (type: "type" | "isRead") => {
  ...
  const handlePickerSelect = (value: string) => {
    ...
    if (activePicker.type === "type") {
      setFormData({ ...formData, type: value });
    } else if (activePicker.type === "isRead") {
      setFormData({ ...formData, isRead: value });
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Legal Notifications</Text>
          <Text style={styles.headerSubtitle}>Manage your notifications and associated metadata.</Text>
        </View>
        <TouchableOpacity style={[styles.btn, styles.btnPrimary, { marginTop: 8 }]} onPress={() => { resetForm(); setAddOpen(true); }}>
          ...
          <Text style={styles.btnPrimaryText}>Add Notification</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} tintColor={colors.primary} />}>
        ...
        {loading ? <ActivityIndicator size="large" color={colors.primary} /> : filteredItems.length === 0 ? (
          ...
          filteredItems.map((c, index) => {
            const itemKey = c.id || c._id || `notification-item-${index}`;
            return (
              <View key={itemKey} style={styles.itemCard}>
                <View style={styles.cardHeader}>
                  <View style={{ flex: 1, paddingRight: 8 }}>
                    <Text style={styles.itemNumber}>{c.notificationNumber}</Text>
                    <Text style={styles.itemTitle}>{c.title}</Text>
                  </View>
                  <View style={styles.typeBadge}><Text style={{ fontSize: 11, fontWeight: "600", color: colors.primary }}>{c.type || "System"}</Text></View>
                </View>
                <View style={styles.cardDetailsGrid}>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>Message</Text><Text numberOfLines={1} style={[styles.detailText, { flex: 1, textAlign: "right", paddingLeft: 16 }]}>{c.message || "—"}</Text></View>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>Read Status</Text><Text style={styles.detailText}>{c.isRead || "No"}</Text></View>
                </View>
      ...
      <Modal visible={viewOpen} transparent animationType="slide" onRequestClose={() => setViewOpen(false)}>
        <View style={styles.modalOverlay}>
          <ScrollView contentContainerStyle={styles.modalScrollFormContainer}>
            <View style={styles.modalHeaderRow}><Text style={styles.modalTitle}>Notification Details</Text><TouchableOpacity onPress={() => setViewOpen(false)}><X size={20} color={colors.text} /></TouchableOpacity></View>
            {selectedItem && (
              <View style={{ gap: 14, marginVertical: 12 }}>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Notification ID</Text><Text style={styles.viewBlockValue}>{selectedItem.notificationNumber}</Text></View>
                ...
                {selectedItem.message ? <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Message</Text><Text style={styles.viewNotesBox}>{selectedItem.message}</Text></View> : null}
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Type</Text><Text style={styles.viewBlockValue}>{selectedItem.type || "N/A"}</Text></View>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Read Status</Text><Text style={styles.viewBlockValue}>{selectedItem.isRead || "N/A"}</Text></View>
              </View>
            )}
      ...
      <Modal visible={addOpen || editOpen} transparent animationType="slide" onRequestClose={() => { setAddOpen(false); setEditOpen(false); }}>
        <View style={styles.modalOverlay}>
          <ScrollView contentContainerStyle={styles.modalScrollFormContainer}>
            <View style={styles.modalHeaderRow}><Text style={styles.modalTitle}>{addOpen ? "Create New Notification" : `Edit Notification ${selectedItem?.notificationNumber || ""}`}</Text><TouchableOpacity onPress={() => { setAddOpen(false); setEditOpen(false); }}><X size={20} color={colors.text} /></TouchableOpacity></View>
            <View style={{ gap: 12, marginVertical: 8 }}>
              ...
              <View><Text style={styles.fieldLabel}>Message</Text><TextInput style={[styles.modalInput, styles.textAreaInput]} multiline numberOfLines={3} value={formData.message} onChangeText={(text) => setFormData({ ...formData, message: text })} placeholder="Message..." placeholderTextColor={colors.placeholderText} /></View>
              <View><Text style={styles.fieldLabel}>Type *</Text><TouchableOpacity style={styles.formPickerTrigger} onPress={() => openPickerModal("type")}><Text style={styles.formPickerTriggerText}>{formData.type}</Text><ChevronDown size={14} color={colors.textMuted} /></TouchableOpacity></View>
              <View><Text style={styles.fieldLabel}>Read Status</Text><TouchableOpacity style={styles.formPickerTrigger} onPress={() => openPickerModal("isRead")}><Text style={styles.formPickerTriggerText}>{formData.isRead}</Text><ChevronDown size={14} color={colors.textMuted} /></TouchableOpacity></View>
            </View>
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity style={[styles.btn, styles.btnPrimary]} disabled={isSubmitting} onPress={addOpen ? handleAdd : handleEdit}>{isSubmitting ? <ActivityIndicator size="small" color="#FFF" /> : <Text style={styles.btnPrimaryText}>{addOpen ? "Create Notification" : "Save Changes"}</Text>}</TouchableOpacity>
            </View>
          </ScrollView>
        </View>
      </Modal>
