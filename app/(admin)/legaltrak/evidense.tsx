...
interface LegalEvidence {
  ...
  title: string;
  description?: string;
  evidenceType: "Physical" | "Digital" | "Testimonial" | string;
  dateAcquired?: string;
  location?: string;
  caseReference?: string;
  status?: "Logged" | "Under Review" | "Admitted" | string;
...
export default function EvidenceScreen() {
  ...
  const [activePicker, setActivePicker] = useState<{ type: "evidenceType" | "status"; options: { label: string; value: string }[] } | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      const res = await listResource<ApiResponse | LegalEvidence[]>("legal/evidence");
      ...
      Alert.alert("Error", "Failed to load evidence records from backend services.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleEditOpen = (c: LegalEvidence) => {
    ...
    setFormData({
      ...
      evidenceType: c.evidenceType || "Physical",
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const payload: any = { ...formData };
      Object.keys(payload).forEach((k) => { if (payload[k] === "") delete payload[k]; });
      await updateResource<LegalEvidence>("legal/evidence", targetId, payload);
      setEditOpen(false);
      setSelectedItem(null);
      resetForm();
      loadData();
    } catch {
      Alert.alert("Error", "Failed to update target evidence record metrics safely.");
    } finally {
      setIsSubmitting(false);
    }
  };

  const openDeleteConfirmation = (item: LegalEvidence) => {
    const targetId = item.id || item._id || "";
    Alert.alert("Delete Evidence", `Are you sure you want to delete ${item.evidenceNumber}? This cannot be undone.`, [
      { text: "Cancel", style: "cancel" },
      { text: "Delete Permanently", style: "destructive", onPress: async () => {
          try {
            await deleteResource("legal/evidence", targetId);
            loadData();
          } catch {
            Alert.alert("Error", "Exclusion structural operations failed inside storage context mapping mapping.");
          }
        }},
    ]);
  };
  ...
  const openPickerModal = (type: "evidenceType" | "status") => {
  ...
  const handlePickerSelect = (value: string) => {
    ...
    if (activePicker.type === "evidenceType") {
      setFormData({ ...formData, evidenceType: value });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Legal Evidence</Text>
          <Text style={styles.headerSubtitle}>Manage your evidence and associated metadata.</Text>
        </View>
        <TouchableOpacity style={[styles.btn, styles.btnPrimary, { marginTop: 8 }]} onPress={() => { resetForm(); setAddOpen(true); }}>
          ...
          <Text style={styles.btnPrimaryText}>Add Evidence</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} tintColor={colors.primary} />}>
        ...
        {loading ? <ActivityIndicator size="large" color={colors.primary} /> : filteredItems.length === 0 ? (
          ...
          filteredItems.map((c, index) => {
            const itemKey = c.id || c._id || `evidence-item-${index}`;
            return (
              <View key={itemKey} style={styles.itemCard}>
                <View style={styles.cardHeader}>
                  <View style={{ flex: 1, paddingRight: 8 }}>
                    <Text style={styles.itemNumber}>{c.evidenceNumber}</Text>
                    <Text style={styles.itemTitle}>{c.title}</Text>
                  </View>
                  <View style={styles.statusBadge}><Text style={{ fontSize: 11, fontWeight: "600", color: colors.primary }}>{c.status || "Logged"}</Text></View>
                </View>
                <View style={styles.cardDetailsGrid}>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>Evidence Type</Text><Text style={styles.detailText}>{c.evidenceType || "—"}</Text></View>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>Storage Location</Text><Text style={styles.detailText}>{c.location || "—"}</Text></View>
                </View>
      ...
      <Modal visible={viewOpen} transparent animationType="slide" onRequestClose={() => setViewOpen(false)}>
        <View style={styles.modalOverlay}>
          <ScrollView contentContainerStyle={styles.modalScrollFormContainer}>
            <View style={styles.modalHeaderRow}><Text style={styles.modalTitle}>Evidence Details</Text><TouchableOpacity onPress={() => setViewOpen(false)}><X size={20} color={colors.text} /></TouchableOpacity></View>
            {selectedItem && (
              <View style={{ gap: 14, marginVertical: 12 }}>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Evidence No.</Text><Text style={styles.viewBlockValue}>{selectedItem.evidenceNumber}</Text></View>
                ...
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Evidence Type</Text><Text style={styles.viewBlockValue}>{selectedItem.evidenceType || "N/A"}</Text></View>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Date Acquired</Text><Text style={styles.viewBlockValue}>{selectedItem.dateAcquired ? selectedItem.dateAcquired.split("T")[0] : "N/A"}</Text></View>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Storage Location</Text><Text style={styles.viewBlockValue}>{selectedItem.location || "N/A"}</Text></View>
      ...
      <Modal visible={addOpen || editOpen} transparent animationType="slide" onRequestClose={() => { setAddOpen(false); setEditOpen(false); }}>
        <View style={styles.modalOverlay}>
          <ScrollView contentContainerStyle={styles.modalScrollFormContainer}>
            <View style={styles.modalHeaderRow}><Text style={styles.modalTitle}>{addOpen ? "Create New Evidence" : `Edit Evidence ${selectedItem?.evidenceNumber || ""}`}</Text><TouchableOpacity onPress={() => { setAddOpen(false); setEditOpen(false); }}><X size={20} color={colors.text} /></TouchableOpacity></View>
            <View style={{ gap: 12, marginVertical: 8 }}>
              ...
              <View><Text style={styles.fieldLabel}>Evidence Type *</Text><TouchableOpacity style={styles.formPickerTrigger} onPress={() => openPickerModal("evidenceType")}><Text style={styles.formPickerTriggerText}>{formData.evidenceType}</Text><ChevronDown size={14} color={colors.textMuted} /></TouchableOpacity></View>
              <View><Text style={styles.fieldLabel}>Date Acquired</Text><TextInput style={styles.modalInput} value={formData.dateAcquired} onChangeText={(text) => setFormData({ ...formData, dateAcquired: text })} placeholder="YYYY-MM-DD" placeholderTextColor={colors.placeholderText} /></View>
              <View><Text style={styles.fieldLabel}>Storage Location</Text><TextInput style={styles.modalInput} value={formData.location} onChangeText={(text) => setFormData({ ...formData, location: text })} placeholder="Storage Location" placeholderTextColor={colors.placeholderText} /></View>
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity style={[styles.btn, styles.btnPrimary]} disabled={isSubmitting || uploadingFiles} onPress={addOpen ? handleAdd : handleEdit}>{isSubmitting ? <ActivityIndicator size="small" color="#FFF" /> : <Text style={styles.btnPrimaryText}>{addOpen ? "Create Evidence" : "Save Changes"}</Text>}</TouchableOpacity>
            </View>
          </ScrollView>
        </View>
      </Modal>
