...
interface LegalReport {
  ...
  title: string;
  description?: string;
  reportType?: "Financial" | "Case Status" | "Time Tracking" | "Other" | string;
  generatedBy?: string;
...
export default function ReportsScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const res = await listResource<ApiResponse | LegalReport[]>("legal/reports");
      ...
      Alert.alert("Error", "Failed to load reports from backend services.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleAdd = async () => {
    if (!formData.title.trim()) {
      Alert.alert("Validation Error", "Report Title field is required.");
      return;
    }
    try {
  ...
  const handleEditOpen = (c: LegalReport) => {
    ...
    setFormData({
      ...
      reportType: c.reportType || "Case Status",
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const payload: any = { ...formData };
      Object.keys(payload).forEach((k) => { if (payload[k] === "") delete payload[k]; });
      await updateResource<LegalReport>("legal/reports", targetId, payload);
      setEditOpen(false);
      setSelectedItem(null);
      resetForm();
      loadData();
    } catch {
      Alert.alert("Error", "Failed to update target report record metrics safely.");
    } finally {
      setIsSubmitting(false);
    }
  };

  const openDeleteConfirmation = (item: LegalReport) => {
    const targetId = item.id || item._id || "";
    Alert.alert("Delete Report", `Are you sure you want to delete ${item.reportNumber}? This cannot be undone.`, [
      { text: "Cancel", style: "cancel" },
      { text: "Delete Permanently", style: "destructive", onPress: async () => {
          try {
            await deleteResource("legal/reports", targetId);
            loadData();
          } catch {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Legal Reports</Text>
          <Text style={styles.headerSubtitle}>Manage your reports and associated metadata.</Text>
        </View>
        <TouchableOpacity style={[styles.btn, styles.btnPrimary, { marginTop: 8 }]} onPress={() => { resetForm(); setAddOpen(true); }}>
          ...
          <Text style={styles.btnPrimaryText}>Add Report</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} tintColor={colors.primary} />}>
        ...
        {loading ? <ActivityIndicator size="large" color={colors.primary} /> : filteredItems.length === 0 ? (
          ...
          filteredItems.map((c, index) => {
            const itemKey = c.id || c._id || `report-item-${index}`;
            return (
              <View key={itemKey} style={styles.itemCard}>
                <View style={styles.cardHeader}>
                  <View style={{ flex: 1, paddingRight: 8 }}>
                    <Text style={styles.itemNumber}>{c.reportNumber}</Text>
                    <Text style={styles.itemTitle}>{c.title}</Text>
                  </View>
                  <View style={styles.statusBadge}><Text style={{ fontSize: 11, fontWeight: "600", color: colors.primary }}>{c.reportType || "Case Status"}</Text></View>
                </View>
                <View style={styles.cardDetailsGrid}>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>Generated By</Text><Text style={styles.detailText}>{c.generatedBy || "—"}</Text></View>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>Date Generated</Text><Text style={styles.detailText}>{c.dateGenerated ? new Date(c.dateGenerated).toLocaleDateString() : "N/A"}</Text></View>
                </View>
        ...
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setActivePicker(null)}>
          <View style={styles.pickerContentContainer}>
            ...
            {activePicker?.options.map((opt, idx) => (
              <TouchableOpacity key={idx} style={styles.pickerOptionItem} onPress={() => { setFormData({ ...formData, reportType: opt.value }); setActivePicker(null); }}><Text style={styles.pickerOptionText}>{opt.label}</Text></TouchableOpacity>
            ))}
          </View>
        </TouchableOpacity>
      </Modal>
      ...
      <Modal visible={viewOpen} transparent animationType="slide" onRequestClose={() => setViewOpen(false)}>
        <View style={styles.modalOverlay}>
          <ScrollView contentContainerStyle={styles.modalScrollFormContainer}>
            <View style={styles.modalHeaderRow}><Text style={styles.modalTitle}>Report Details</Text><TouchableOpacity onPress={() => setViewOpen(false)}><X size={20} color={colors.text} /></TouchableOpacity></View>
            {selectedItem && (
              <View style={{ gap: 14, marginVertical: 12 }}>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Report No.</Text><Text style={styles.viewBlockValue}>{selectedItem.reportNumber}</Text></View>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Report Title</Text><Text style={styles.viewBlockValue}>{selectedItem.title || "N/A"}</Text></View>
                ...
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Type</Text><Text style={styles.viewBlockValue}>{selectedItem.reportType || "N/A"}</Text></View>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Generated By</Text><Text style={styles.viewBlockValue}>{selectedItem.generatedBy || "N/A"}</Text></View>
                <View style={styles.viewDetailGridBlock}><Text style={styles.viewBlockLabel}>Date Generated</Text><Text style={styles.viewBlockValue}>{selectedItem.dateGenerated ? new Date(selectedItem.dateGenerated).toLocaleDateString() : "N/A"}</Text></View>
              </View>
            )}
      ...
      <Modal visible={addOpen || editOpen} transparent animationType="slide" onRequestClose={() => { setAddOpen(false); setEditOpen(false); }}>
        <View style={styles.modalOverlay}>
          <ScrollView contentContainerStyle={styles.modalScrollFormContainer}>
            <View style={styles.modalHeaderRow}><Text style={styles.modalTitle}>{addOpen ? "Create New Report" : `Edit Report ${selectedItem?.reportNumber || ""}`}</Text><TouchableOpacity onPress={() => { setAddOpen(false); setEditOpen(false); }}><X size={20} color={colors.text} /></TouchableOpacity></View>
            <View style={{ gap: 12, marginVertical: 8 }}>
              <View><Text style={styles.fieldLabel}>Report Title *</Text><TextInput style={styles.modalInput} value={formData.title} onChangeText={(text) => setFormData({ ...formData, title: text })} placeholder="Report Title" placeholderTextColor={colors.placeholderText} /></View>
              ...
              <View><Text style={styles.fieldLabel}>Type *</Text><TouchableOpacity style={styles.formPickerTrigger} onPress={openPickerModal}><Text style={styles.formPickerTriggerText}>{formData.reportType}</Text><ChevronDown size={14} color={colors.textMuted} /></TouchableOpacity></View>
              <View><Text style={styles.fieldLabel}>Generated By</Text><TextInput style={styles.modalInput} value={formData.generatedBy} onChangeText={(text) => setFormData({ ...formData, generatedBy: text })} placeholder="Generated By" placeholderTextColor={colors.placeholderText} /></View>
              <View><Text style={styles.fieldLabel}>Date Generated</Text><TextInput style={styles.modalInput} value={formData.dateGenerated} onChangeText={(text) => setFormData({ ...formData, dateGenerated: text })} placeholder="YYYY-MM-DD" placeholderTextColor={colors.placeholderText} /></View>
            </View>
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity style={[styles.btn, styles.btnPrimary]} disabled={isSubmitting} onPress={addOpen ? handleAdd : handleEdit}>{isSubmitting ? <ActivityIndicator size="small" color="#FFF" /> : <Text style={styles.btnPrimaryText}>{addOpen ? "Create Report" : "Save Changes"}</Text>}</TouchableOpacity>
            </View>
          </ScrollView>
        </View>
      </Modal>
