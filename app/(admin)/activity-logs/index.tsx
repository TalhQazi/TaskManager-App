...
export default function ActivityLogs() {
  ...
  const activeColors = useMemo(() => {
    const bg =
      ...
      (isLightTheme ? "#f8fafc" : "#09090b");
    const surface =
      uiTheme?.panelColors?.dashboardCardBackground ||
      (isLightTheme ? "#ffffff" : "#141417");
    const textColor =
      uiTheme?.panelColors?.dashboardTextColor ||
      (isLightTheme ? "#0f172a" : "#f4f4f5");
    const textSecondary = isLightTheme ? "#64748b" : "#9ca3af";
    const border =
      uiTheme?.panelColors?.borderColor ||
      (isLightTheme ? "#e2e8f0" : "rgba(255, 255, 255, 0.08)");
    const primary =
      uiTheme?.customColors?.primary || Colors?.primary || "#b45309";
  ...
  const styles = useMemo(
    () => createStyles(activeColors, isTablet, isSmallScreen, height),
    [activeColors, isTablet, isSmallScreen, height]
  );
  ...
  const fetchLogs = useCallback(
    ...
    [currentPage, selectedUserId, actionFilter, resourceTypeFilter, dateFrom, dateTo, searchQuery]
  );
  ...
  const getActionStyles = (action: string) => {
    return (
      ACTION_COLORS[action] || {
        bg: activeColors.inputBg,
        text: activeColors.textSecondary,
      }
    );
  };
  ...
  const activeResourceLabel =
    resourceOptions.find((r) => r.id === resourceTypeFilter)?.name || "All Resources";

  return (
    <SafeAreaView style={s(styles.safeArea)}>
      <ScrollView
        contentContainerStyle={s(styles.viewportScrollCanvas)}
      ...
      >
          ...
          <View style={s(styles.headerMainTitleRowAlignment)}>
            ...
            >
              <RefreshCw
                ...
                style={loading ? s(styles.loadingIconRotationOpacity) : undefined}
              />
        ...
        {summary && (
          <View style={s(styles.statsSummaryGridDashboardStrip)}>
            ...
            <View style={s(styles.statMetricDataCard)}>
              ...
              <Text style={s(styles.statMetricStringValueText)} numberOfLines={1}>
                {summary.actionCounts[0]?.action
                  ? ACTION_LABELS[summary.actionCounts[0].action] || summary.actionCounts[0].action
                  : "N/A"}
              </Text>
            </View>
            ...
            <View style={s(styles.statMetricDataCard)}>
              <View style={s(styles.statMetricCardHeaderWrapperRow)}>
                <Filter size={16} color={activeColors.primary} />
        ...
        <View style={s(styles.sectionModuleCardFrame)}>
          ...
          {usersLoading ? (
            <ActivityIndicator
              size="small"
              color={activeColors.primary}
              style={s(styles.loaderInlineSpacing)}
            />
          ) : (
              ...
              {[
                ...
                return (
                  <View key={group.title} style={s(styles.userCategoryBlockGroup)}>
                    <Text style={s(styles.userCategoryGroupSectionTitle)}>
                      {group.title}
                    </Text>
                        ...
                        return (
                          <TouchableOpacity
                            ...
                            style={[
                              ...
                              isTargetSelected
                                ? s(styles.userSelectionActiveBorder)
                                : s(styles.userSelectionInactiveBorder),
                            ]}
                            activeOpacity={0.7}
                          ...
                          >
                            <View
                              style={[
                                s(styles.userIconWrapperBoxCircle),
                                isTargetSelected && s(styles.userIconActiveWrapperBg),
                              ]}
                            >
                              <User
                                size={14}
                                color={isTargetSelected ? "#ffffff" : activeColors.primary}
                              />
                            </View>
                            <View style={s(styles.userMetadataTextFlexBlock)}>
                              <Text
                                style={s(styles.userProfileFullNameDisplayLabelText)}
                                numberOfLines={1}
                              >
                                {u.name || u.username}
                              </Text>
                              <Text
                                style={s(styles.userProfileSystemUniqueUsernameText)}
                                numberOfLines={1}
                              >
              ...
              <View style={s(styles.userSelectionActionStripRowButtons)}>
                ...
                >
                  <Calendar
                    ...
                    style={s(styles.marginRightIconPadding)}
                  />
                  <Text style={s(styles.pillContextActionControlWellButtonText)}>
                    Today
                  </Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  style={[
                    s(styles.pillContextActionControlWellButton),
                    s(styles.pillContextDangerBorderAlertClearBg),
                  ]}
                  activeOpacity={0.7}
                  onPress={() => {
                    setSelectedUserId("");
                ...
                >
                  <Text
                    style={[
                      s(styles.pillContextActionControlWellButtonText),
                      s(styles.pillContextDangerText),
                    ]}
                  >
                    Clear Selection
                  </Text>
        ...
        <View style={s(styles.sectionModuleCardFrame)}>
          ...
          <View style={s(styles.filterMenuDropdownsGridArrangementRow)}>
            ...
            >
              <Text
                style={s(styles.dropdownMenuSelectionDisplayValueText)}
                numberOfLines={1}
              >
                {activeActionLabel}
              </Text>
          ...
          <TouchableOpacity
            style={s(styles.masterFilterClearStripButtonAnchor)}
            onPress={clearFilters}
            activeOpacity={0.7}
          >
            <X size={14} color="#dc2626" style={s(styles.marginRightIconPadding)} />
            <Text style={s(styles.masterFilterClearStripButtonText)}>
        ...
        <View style={[s(styles.sectionModuleCardFrame), s(styles.noPaddingLayoutOverrider)]}>
          <View style={s(styles.registrySectionHeadingStripBlockHeader)}>
            <Text style={s(styles.registrySectionHeadingTextTitle)}>
              Activity Log ({totalItems.toLocaleString()} total)
            </Text>
          </View>
          ...
          {loading && logs.length === 0 ? (
            <ActivityIndicator
              size="large"
              color={activeColors.primary}
              style={s(styles.loaderCentralSpacingCanvas)}
            />
          ) : logs.length === 0 ? (
            <View style={s(styles.emptyRegistryFallbackGraphicLayoutCenter)}>
              <FileText
                size={40}
                color={activeColors.textSecondary}
                style={s(styles.emptyRegistryCentralIconSpacing)}
              />
              <Text style={s(styles.emptyRegistryPrimaryMessageHeadingText)}>
                No activity logs found
              </Text>
              <Text style={s(styles.emptyRegistrySecondaryMessageParagraphText)}>
                Try adjusting your filters or wait for new system events.
              </Text>
            </View>
          ) : (
                  ...
                  <View key={log.id} style={s(styles.registryItemRowCellContainerTile)}>
                    <View style={s(styles.registryRowCellTopMetaBarFlexLine)}>
                      <View style={s(styles.registryActorIdentityFlexWrapperRow)}>
                        ...
                        />
                        <View style={styles.flexFill}>
                          <Text style={s(styles.registryActorUsernameBoldHeaderText)}>
                            {log.actorUsername || "Unknown"}
                          </Text>
                          <Text style={s(styles.registryActorRoleSubtitleText)}>
                            {log.actorRole}
                          </Text>
                        </View>
                      </View>
                      <View
                        style={[
                          s(styles.actionPillBadgeFrameBlock),
                          { backgroundColor: badgeStyle.bg },
                        ]}
                      >
                        <Text
                          style={[
                            s(styles.actionPillBadgeInnerText),
                    ...
                    <View style={s(styles.registryRowCellMiddlePayloadDisplayBlock)}>
                      ...
                      <View style={s(styles.resourceMetadataLabelsTextColumn)}>
                        ...
                        {log.resourceName ? (
                          <Text
                            style={s(styles.resourceUniqueIdentitySystemNameText)}
                            numberOfLines={1}
                          >
                            {log.resourceName}
                          </Text>
                    ...
                    {log.description ? (
                      <Text
                        style={s(styles.resourceDescriptionParagraphBlockText)}
                        numberOfLines={3}
                      >
                        {log.description}
                      </Text>
                    ...
                    <View style={s(styles.registryRowCellFooterChronologyBarLine)}>
                      <Text style={s(styles.chronologyTimestampLabelText)}>
                        {formatDate(log.createdAt)}
                      </Text>
                      <View style={s(styles.ipNetworkAddressCodePillBadge)}>
                        <Text style={s(styles.ipNetworkAddressInnerTextString)}>
          ...
          {totalPages > 1 && (
            <View style={s(styles.paginationControlsFooterBarLayoutBlock)}>
              <TouchableOpacity
                style={[
                  s(styles.paginationArrowStepBoundaryButton),
                  currentPage === 1 && s(styles.paginationArrowDisabledOpacity),
                ]}
              ...
              <TouchableOpacity
                style={[
                  s(styles.paginationArrowStepBoundaryButton),
                  currentPage === totalPages && s(styles.paginationArrowDisabledOpacity),
                ]}
          ...
          <View style={s(styles.pickerContentWindowCardSurface)}>
            <View style={s(styles.pickerContentHeaderBarTopRow)}>
              ...
              <TouchableOpacity
                ...
                style={s(styles.pickerCloseCrossTouchTargetBoundary)}
                activeOpacity={0.7}
              >
            ...
            <ScrollView
              style={s(styles.pickerSelectionItemsScrollCanvasList)}
              ...
              {activePicker === "action" ? (
                <>
                  ...
                  >
                    <Text
                      style={[
                        s(styles.pickerSelectionOptionValueLabelString),
                        actionFilter === "all" && s(styles.pickerSelectionActiveGoldText),
                      ]}
                    >
                      All Actions
                    </Text>
                  ...
                  {Object.entries(ACTION_LABELS).map(([key, label]) => (
                    ...
                    >
                      <Text
                        style={[
                          s(styles.pickerSelectionOptionValueLabelString),
                          actionFilter === key && s(styles.pickerSelectionActiveGoldText),
                        ]}
                      >
                        {label}
                      </Text>
                    ...
                    <Text
                      style={[
                        ...
                        resourceTypeFilter === option.id &&
                          s(styles.pickerSelectionActiveGoldText),
                      ]}
                    >
                      {option.name}
                    </Text>
...
const createStyles = (
  ...
  StyleSheet.create({
    ...
    viewportScrollCanvas: {
      paddingHorizontal: isTablet ? 24 : isSmallScreen ? 10 : 16,
      paddingBottom: 40,
    },
