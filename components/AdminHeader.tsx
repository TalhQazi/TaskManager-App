...
export default function AdminHeader({ onMenuPress }: { onMenuPress: () => void }) {
  ...
  const { data: headerSettings } = useQuery<HeaderSettings | null>({
    ...
    queryFn: async () => {
      try {
        ...
        return res.data?.item || (res.data as any) || null;
      } catch (e) {
        return null;
      }
    },
  ...
  // Sync server position when user is not actively editing
  useEffect(() => {
  ...
  const handlePickHeaderImage = async () => {
    try {
      ...
      if (!pickerResult.canceled && pickerResult.assets && pickerResult.assets.length > 0) {
        ...
        queryClient.invalidateQueries({ queryKey: ["AdminHeaderSettings"] });
              ...
              <View style={styles.avatarWrapper}>
                <View style={[styles.avatarRingFrame, { borderColor: statusColor }]}>
                  {avatarImageSource && !avatarLoadError ? (
                    <Image source={avatarImageSource} style={styles.avatarAsset} resizeMode="cover" onError={() => setAvatarLoadError(true)} />
                  ) : (
