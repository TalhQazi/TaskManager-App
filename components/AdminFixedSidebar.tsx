...
function buildColors(uiTheme: any) {
  ...
  return {
    background: uiTheme?.panelColors?.dashboardBackground || (isDark ? '#090d16' : '#ffffff'),
    cardBg: uiTheme?.panelColors?.dashboardCardBackground || (isDark ? '#111827' : '#f8fafc'),
    text: uiTheme?.panelColors?.dashboardTextColor || (isDark ? '#f4f4f5' : '#0f172a'),
    textSecondary: isDark ? '#a1a1aa' : '#64748b',
    border: uiTheme?.panelColors?.borderColor || (isDark ? '#27272a' : '#e2e8f0'),
...
const MENU_ITEMS = [
  ...
  },
  {
    ...
    children: [
      ...
      { icon: BarChart, label: "Reports", path: "/(admin)/legaltrak/reports" },
...
export default function AdminFixedSidebar({ isOpen, onClose }: any) {
  ...
  const colors = React.useMemo(() => buildColors(uiTheme), [uiTheme]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Failed to load token in admin sidebar:', err);
      }
    })();
  ...
  const rawAvatarPath = React.useMemo(() => {
    return (
            ...
            <View key={item.id} style={styles.submenuContainer}>
              ...
              >
                ...
                {hasChildren ? (
                  ...
                  isParentActive && <ChevronRight color={colors.primary} size={16} />
                )}
              </TouchableOpacity>
              ...
              {hasChildren && isExpanded && (
                <View style={styles.submenuChildrenBlock}>
                  {item.children?.map((child, index) => {
                    ...
                    const childIconColor = isChildActive ? colors.primary : colors.textSecondary;
