  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.warn("[adminTaskApi] /api/employees error", e);
      }
      ...
      try {
        ...
        console.warn("[adminTaskApi] /api/users/all error", e);
      }
...
export function useUpdateTask() {
  ...
  return useMutation({
    mutationFn: async ({ taskId, payload }: { taskId: string; payload: Partial<Task> }) =>
      apiFetch(`/api/tasks/${encodeURIComponent(taskId)}`, { method: "PUT", body: JSON.stringify(payload) }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["tasks"] });
      qc.invalidateQueries({ queryKey: ["projects"] });
    },
...
export function useDeleteTaskAttachment() {
  ...
  return useMutation({
    mutationFn: async ({ taskId, attachmentIndex }: { taskId: string; attachmentIndex: number }) => {
      try {
        return await apiFetch(`/api/tasks/${encodeURIComponent(taskId)}/attachments/${attachmentIndex}`, {
          method: "DELETE",
        });
      } catch {
        return await apiFetch(`/api/tasks/${encodeURIComponent(taskId)}/attachments/${attachmentIndex}/archive`, {
          method: "POST",
        });
      }
    },
...
export function useUpdateProject() {
  ...
  return useMutation({
    mutationFn: async ({ projectId, payload }: { projectId: string; payload: Partial<Project> }) =>
      apiFetch(`/api/projects/${encodeURIComponent(projectId)}`, { method: "PUT", body: JSON.stringify(payload) }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["projects"] });
      qc.invalidateQueries({ queryKey: ["tasks"] });
    },
...
export function useAddProjectAttachments() {
  ...
  return useMutation({
    mutationFn: async ({ projectId, attachments }: { projectId: string; attachments: any[] }) =>
      apiFetch(`/api/projects/${encodeURIComponent(projectId)}`, { method: "PUT", body: JSON.stringify({ attachments }) }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["projects"] });
      qc.invalidateQueries({ queryKey: ["tasks"] });
    },
...
export function useDeleteProjectAttachment() {
  ...
  return useMutation({
    mutationFn: async ({ projectId, attachmentIndex }: { projectId: string; attachmentIndex: number }) => {
      try {
        return await apiFetch(`/api/projects/${encodeURIComponent(projectId)}/attachments/${attachmentIndex}`, {
          method: "DELETE",
        });
      } catch {
        return await apiFetch(`/api/projects/${encodeURIComponent(projectId)}/attachments/${attachmentIndex}/archive`, {
          method: "POST",
        });
      }
    },
